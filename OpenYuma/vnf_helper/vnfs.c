...
/*
 *   ---------------------- DATASTRUCTURE -------------------------
 * This helper function will clean up the vnf's hashtable connected_switches.
 * It will bring down interfaces, remove ports from switches, and destroy
 * virtual ethernet pairs
 * gchar* veth_pair_as_char - key in the hashtable ( e.g., uny0-uny1)
 * gchar* value - associated value in the hashtable (GSList* containing sw_id
 * and sw_port - however, it is innecessary to use for deleting
 * gchar* vnf_id - the id of the vnf - it is an additional argument in order to
 * be able to call other helper functions that require corresponding vnf ids
 */
//void clearVethPairsHashTable(gchar* veth_pair_as_char,
//							 GSList* sw_data,
//							 gchar* vnf_id)
//{
//
//
//	//bringing down virtual ethernet pairs
//	custom_log(INFO, "\tDisconnecting virtual ethernet pairs...");
//	disconnectVNF(vnf_id, veth_pair_as_char);
//	custom_log(INFO, "\t\tDONE");
//
//	//deleting switch port - the second element of veth_pair_as_char
//	//is connected to a switch in all cases
//	gchar* veth_dev = subStringDeviceIdFromVirtualEthernetPair(
//															veth_pair_as_char,
//															1);
//	custom_log(INFO, "\tDeleting switch port...");
//	deleteSwitchPort(veth_dev);
//	custom_log(INFO, "\t\tDONE");
//
//
//
//	//deleting virtual ethernet pair
//	custom_log(INFO, "\tDeleting virtual ethernet pairs...");
//	deleteVirtualEthernetPair(veth_pair_as_char);
//	custom_log(INFO, "\t\tDONE");
//
//
//}
...
/*
 *     ---------------------- DATASTRUCTURE -------------------------
 * This procedure adds a GHashtable element as a value to the given GHashTable
 * identified by a string key.
 * GHashTable *t - the hashtable object to insert the data
 * gchar *key - key
 * GHashTable *nested_table - hashtable to be nested
 * gboolean supress - TRUE = supress logging, FALSE - verbose mode
 */
//void add_hashtable_element(GHashTable *t,
//						   gchar *key,
//						   GHashTable *nested_table,
//						   gboolean supress)
//{
//	//I do not know the reason, probably a bug, but if a key exists then the
//	//g_hash_table_insert() does not update it, but rather cause error.
...
//	if(g_hash_table_contains(t,key))
//	{
//		//Hashtable already contains data identified by the given key
//		custom_log(WARNING, g_strdup_printf("[VNF_DATASTRUCTURE] "
//								   "[%s] as key already exists!\n"
//								   "We need to remove it first!",
//								   key));
//		//so, we remove that data
//		g_hash_table_remove(t, key);
//	}
//
//	//insert new data to the hashtable
//    int success  =  g_hash_table_insert(t,
//										g_strdup (key),
//										nested_table);
//	if(success)
...
//					 "Nested hashtable is added");
//		}
//	}
//	else
//	{
//		//if something went wrong, we do not care about supress variable,
//		//we always print out that an error occurred
//		custom_log(ERROR, "[VNF_DATASTRUCTURE] "
//				  "Error during adding nested hashtable");
//	}
//}

/*
 *     ---------------------- DATASTRUCTURE -------------------------
 * This procedure adds a GSList element as a value to the given GHashTable
 * identified by a string key.
 * GHashTable *t - the hashtable object to insert the data
 * gchar *key - key
 * GSList *list - GSList element to be added
 * gboolean supress - TRUE = supress logging, FALSE - verbose mode
 */
//void add_list_element(GHashTable *t, gchar *key, GSList *list, gboolean supress)
//{
//	//I do not know the reason, probably a bug, but if a key exists then the
//	//g_hash_table_insert() does not update it, but rather cause error.
//	//However, its documentation says that it should do this thing
...
//		//we always print out that an error occurred
//		custom_log(ERROR, "[VNF_DATASTRUCTURE] "
//				  "Error during adding nested list");
//	}
//}
